...
async def get_test_any(test_code):
    pool = await get_pool()
    async with pool.acquire() as conn:
        return await conn.fetchrow("SELECT * FROM tests WHERE test_code=$1", test_code)
...
async def get_all_tests(admin_id):
    pool = await get_pool()
    async with pool.acquire() as conn:
        return await conn.fetch(
            "SELECT * FROM tests WHERE created_by=$1 ORDER BY created_at DESC LIMIT 20",
            admin_id
        )
...
async def end_test(test_code):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE tests SET is_active=FALSE, ended_at=NOW() WHERE test_code=$1", test_code
        )
...
async def get_submission(test_code, user_id):
    pool = await get_pool()
    async with pool.acquire() as conn:
        return await conn.fetchrow(
